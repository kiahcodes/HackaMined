...
const BASE_URL = "http://10.21.15.106:8000";
...
function Chatbot() {
  ...
  const QUICK = [
    { label: "Why is my risk score high?", key: "risk" },
    { label: "What jobs are safer for me?", key: "safer" },
    { label: "Show me 3-month learning paths", key: "3month" },
    { label: "How many BPO jobs are in Indore?", key: "bpo" },
    { label: "मुझे क्या करना चाहिए?", key: "hindi" },
  ];
  ...
  const send = (text, key) => {
    ...
    setTimeout(() => {
      ...
      if (!rk) {
        ...
        if (lo.includes("risk") || lo.includes("score")) rk = "risk";
        else if (lo.includes("safe") || lo.includes("jobs")) rk = "safer";
        else if (lo.includes("3 month") || lo.includes("3-month") || lo.includes("quick")) rk = "3month";
        else if (lo.includes("bpo") || lo.includes("indore")) rk = "bpo";
        else if (lo.includes("क्या") || lo.includes("करना")) rk = "hindi";
        else rk = "risk";
      }
      const resp = BOT_RESP[rk];
      const reply = lang === "hi" ? (resp.hi || resp.en) : resp.en;
      setThinking(false);
      ...
      {open && (
        <div className="fixed bottom-24 right-6 z-50 w-96 rounded-2xl shadow-2xl flex flex-col overflow-hidden"
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b"
            ...
            <div className="flex items-center gap-2"><Dot /><span className="text-sm font-bold" style={{ color: t.textPri }}>AI Career Advisor</span></div>
          ...
          {msgs.length <= 1 && (
            <div className="px-3 pb-2 flex flex-wrap gap-1">
              {QUICK.map(p => (
                <button key={p.key} onClick={() => send(p.label, p.key)}
                  className="text-xs px-2 py-1 rounded-full border transition-all hover:bg-blue-500/25"
          ...
          <div className="flex gap-2 px-3 py-3 border-t" style={{ borderColor: t.border }}>
            ...
            />
            <button onClick={() => send()}
              className="w-8 h-8 rounded-xl bg-blue-500 flex items-center justify-center hover:bg-blue-400 transition-all flex-shrink-0">
...
function Landing({ go }) {
  const t = useT();
  const { dark } = useTheme();
  const jobs = useCounter(2847293), cities = useCounter(22), skills = useCounter(4821), sigs = useCounter(18472);
  const stats = [
...
function WorkerPage() {
  ...
  if (Array.isArray(d.worker_skills)) {
    setWSkills(d.worker_skills);
  }

  if (Array.isArray(d.market_skills)) {
    setMSkills(d.market_skills);
  }

  if (Array.isArray(d.skill_labels)) {
    setSkillLabels(d.skill_labels);
  }
  ...
  console.warn("analyze-worker API failed:", workerResult.reason?.message);
      ...
      if (coursesResult.status === "fulfilled") {
        ...
        if (Array.isArray(data) && data.length > 0) {
          // Normalise: add month index if missing, keep all other fields
          setCourses(data.map((c, i) => ({ month: c.month ?? i + 1, free: c.free ?? false, ...c })));
        }
        ...
        <header className="sticky top-0 z-30 px-8 py-3.5 flex items-center justify-between"
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 rounded-lg px-3 py-1.5"
              ...
              <span className="text-xs font-bold text-orange-400">⚡ 18,472 signals today</span>
            </div>
            <ThemeToggle />
          </div>
        </header>
